...
> **注意：** 二进制文件未进行代码签名。请参阅[未签名二进制文件](#未签名二进制文件)了解各平台的运行方法。
...
发布的二进制文件**未进行代码签名**，这意味着首次运行应用时操作系统可能会显示安全警告。这对于在官方应用商店之外分发的开源软件来说是正常的。

### macOS

macOS 会显示警告："无法打开 App，因为无法验证开发者"。

**解决方法：**

1. **右键菜单方法**（推荐）：
   - 右键点击（或按住 Control 点击）Clipper.app
   - 从上下文菜单中选择"打开"
   - 在弹出的对话框中点击"打开"
   - 之后应用将被记住，可以正常打开
